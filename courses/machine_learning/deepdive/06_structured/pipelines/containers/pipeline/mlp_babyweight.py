...
def preprocess_train_and_deploy(
    project='confluent-98f26',
    bucket='confluent-98f26-kfpdemo',
    start_year='2000'
):
    ...
    preprocess = dsl.ContainerOp(
          ...
          image='gcr.io/confluent-98f26/babyweight-pipeline-bqtocsv:latest',
          arguments=[
            '--project', project,
            '--mode', 'cloud',
            '--bucket', bucket,
    ...
    hparam_train = dsl.ContainerOp(
        name='hypertrain',
        # image needs to be a compile-time string
        image='gcr.io/confluent-98f26/babyweight-pipeline-hypertrain:latest',
        arguments=[
    ...
    deploy_app = dsl.ContainerOp(
          name='deployapp',
          # image needs to be a compile-time string
          image='gcr.io/confluent-98f26/babyweight-pipeline-deployapp:latest',
          arguments=[
...
def train_and_deploy_helper(preprocess, hparam_train):
    ...
    train_tuned = dsl.ContainerOp(
      name='traintuned',
      # image needs to be a compile-time string
      image='gcr.io/confluent-98f26/babyweight-pipeline-traintuned:latest',
      arguments=[
    ...
    deploy_cmle = dsl.ContainerOp(
      name='deploycmle',
      # image needs to be a compile-time string
      image='gcr.io/confluent-98f26/babyweight-pipeline-deploycmle:latest',
      arguments=[
